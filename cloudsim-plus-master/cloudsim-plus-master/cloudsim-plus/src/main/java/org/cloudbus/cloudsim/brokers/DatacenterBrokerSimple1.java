 ...
 */
public class DatacenterBrokerSimple1 extends DatacenterBrokerAbstract {
     ...
     * @post $none
     */
    public DatacenterBrokerSimple1(CloudSim simulation, double sendDelay) {
        super(simulation);
    ...
    protected void requestDatacenterToCreateWaitingVms(Datacenter datacenter) {
        int requestedVms = 0;
        //println(String.format("%.2f", getSimulation().clock()));
        for (final Vm vm :getVmWaitingList()) {
            if (!getVmsToDatacentersMap().containsKey(vm) && !getVmCreationRequestsMap().containsKey(vm)) {
                ...
                send(datacenter.getId(), sendDelay,CloudSimTags.VM_CREATE_ACK, vm);
                
                addVmCreationRequestsMap(vm, datacenter);
                requestedVms++;
            }
        }
        addDatacenterRequestedList(datacenter);
